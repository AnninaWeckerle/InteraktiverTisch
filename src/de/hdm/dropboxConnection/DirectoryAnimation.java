...
/**
 * Klasse, die den Download und Upload von Dateien visualisiert, indem 
 * Symbole (Ordner / Dateien) mit den entsprechenden Dateinamen herein- 
 * oder herausfliegen.
 */
public class DirectoryAnimation {
    ...
    public DirectoryAnimation(String dirName, String typ) {
        ...
        if (dir.isDirectory()) {
            ...
            if (typ.equals("Upload")) {
            	runAnimationUpload();
            }
         
            if (typ.equals("Download")) {
                runAnimationDownload();
            }
            
            else if (typ.equals("UploadNeu")) {
            	runAnimationUploadTry();
            }
        }
    }
    ...
    public static void main(String[] args) {
        if (args.length > 0) {
        		  new DirectoryAnimation(args[0], args[0]);
    ...
    private void runAnimationUpload() {
        try {
            shrinkDir();
    ...
    private void runAnimationDownload() {
        try {
            spreadDir();
        } catch (InterruptedException e) {
            e.printStackTrace();
    ...
    private void runAnimationUploadTry() {
    	 try {
             spreadAndShrinkDir();
         } catch (InterruptedException e) {
             e.printStackTrace();
    ...
    private void spreadAndShrinkDir() throws InterruptedException {
